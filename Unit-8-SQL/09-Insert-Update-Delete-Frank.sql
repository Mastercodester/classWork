...
delete from pet_types;  -- Delete all existing rows from the table
   delete from pet_owner;                     -- ONLY DO THIS IF YOU WANT TO EMPTY THE TABLE!
...
Values('james', (select pet_type_id from pet_types where name = 'Xolo' and species = 'Dog')  
, (select owner_id from pet_owner where first_name = 'Frank' and last_name = 'Fella')),
      
	  ('Vito' , (select pet_type_id from pet_types where name = 'German Shepard ' and species = 'Dog'), 
	  (select owner_id from pet_owner where first_name = 'Vito' and last_name = 'Corelone')),
     
	 ('Kevin', (select pet_type_id from pet_types where name = 'Xolo' and species = 'Dog'),
	  (select owner_id from pet_owner where first_name = 'Kevin' and last_name = 'Gibbs II'))
;
select * from pet;

--pet_type_id was automatically generated by DBMS
--ask thdata base manager for value using select
...
-- select pet_type_id from pet_types where name = 'German Sherpard'

--select pet_type_id from pet_types where name = 'German Shepard ' and species = 'Dog';



--select pet_owner from pet_owner where first_name = 'Frank' and last_name = 'Fella';

-- UPDATE- how we change data in the data in a table 
--
--syntax : Update table
--	set column - new-value
--	where where -condition
--
--The where clause controls how many rows are updated
--
--if there is no where clause all rows  in the table are updated

--
--Always code where clause to tell it what rows you want updated
--(unless your goal is update all the rows in the table)
--
...
--or have the data base tool (pgAdmin) display the tables after updates
--
--if your update was in correct
--In class, just refresh the tables(run the inserts again)

--CHange princess owner to kevin
--We need to change princess owner id to k


update pet
	set owner_id = (select owner_id from pet_owner where first_name = 'Kevin' and last_name = 'Gibbs II')
where name = 'james'
; 
select * from pet where name ='james';
...
update pet_owner
	...
	;
-- DELETE-- remover rows from a table
--
